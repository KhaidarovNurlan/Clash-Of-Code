...
const CreateTournament = () => {
  ...
  const removeKeyword = (levelIndex, keyword) => {
    const newLevels = [...formData.levels];
    newLevels[levelIndex].required_keywords = newLevels[
      levelIndex
    ].required_keywords.filter((k) => k !== keyword);
    setFormData({
      ...formData,
      levels: newLevels,
    });
  ...
  const validateForm = () => {
    ...
    formData.levels.forEach((level, index) => {
      ...
      if (level.required_keywords.length === 0) {
        newErrors[`level_${index}_keywords`] =
            ...
            <div className="space-y-6">
              {formData.levels.map((level, index) => (
                ...
                >
                  ...
                  <div>
                    <label htmlFor="maxPoints" className="form-label">Points</label>
                    <input
                      id="maxPoints"
                      type="number"
                      value={level.points}
                      ...
                      min="0"
                      className={`form-input ${
                        errors[`level_${index}_points`] ? "border-red-500" : ""
                      }`}
                    />
